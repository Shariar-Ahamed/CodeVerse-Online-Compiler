    ...
    <header className="fixed top-0 left-0 w-full border-b border-[var(--border-color)] bg-[var(--bg-secondary)]/45 z-30 backdrop-blur-xl transition-all duration-300">
      ...
      {mobileMenuOpen && (
        ...
        >
          ...
          <button onClick={() => { navigate('/leaderboard'); setMobileMenuOpen(false); }} className="mobile-nav-link text-left text-[var(--text-secondary)] py-1.5 block focus:outline-none">Leaderboard</button>
          {user && user.role === 'admin' && (
